...
function updateOtpCooldownUI(){
  if(!btnSendLink && !btnSendCode) return;

  const baseLabelLink = "Enviar link";
  const baseLabelCode = "Enviar código";
  ...
  const tick = () => {
    ...
    if(left > 0){
      ...
      if(btnSendLink){
        ...
        btnSendLink.textContent = `${baseLabelLink} (${s}s)`;
      }
      if(btnSendCode){
        btnSendCode.disabled = true;
        btnSendCode.textContent = `${baseLabelCode} (${s}s)`;
      }
      return;
    }
    ...
    if(btnSendLink){
      ...
      btnSendLink.textContent = baseLabelLink;
    }
    if(btnSendCode){
      btnSendCode.disabled = !supabaseClient;
      btnSendCode.textContent = baseLabelCode;
    }
...
// ✅ Enviar CÓDIGO (OTP) — ahora sí hace algo
if(btnSendCode){
  btnSendCode.addEventListener("click", async () => {
    ...
    try{
      ...
      beginOtpCooldown();
      showOtpUI(true);
      showAuthMsg("✅ Listo. Revisa tu correo y escribe aquí el código (6–12 dígitos).");
...
if(btnVerifyCode){
  btnVerifyCode.addEventListener("click", async () => {
    const email = (authEmailEl?.value || "").trim();
    const token = (inpCode?.value || "").trim();
    ...
    if(!token || token.length < 6){
      showAuthMsg("Escribe el código (mínimo 6 dígitos).");
      return;
    }
    ...
    try{
      ...
      // Actualiza estado (PRO/FREE) y perfil
      try{ await ensureProfile(); }catch(_){}
      try{ await refreshMe(); }catch(_){}

      // Refresh account UI
      try{ renderAccountUI(); }catch(_){}

    }catch(err){
      showAuthMsg("❌ " + (err?.message || "Código inválido o expirado."));
    }
  });
}
